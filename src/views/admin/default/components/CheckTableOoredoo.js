...
export default function CheckTableOrange() {
  const columnsData = useMemo(
    () => [
      {
        Header: "NOM",
        accessor: "type", // Remplacez par le nom de la colonne dans votre API
      },
    ...
    async function fetchData() {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_KEY}/api/v1/suppliers/ooredoo`,
          //"http://localhost:8080/api/v1/suppliers/ooredoo"
        );
