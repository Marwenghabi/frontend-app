...
export default function CheckTableOrange() {
  const columnsData = useMemo(
    () => [
      {
        Header: "NOM",
        accessor: "name", // Remplacez par le nom de la colonne dans votre API
      },
      {
        Header: "MESSAGE",
        accessor: "message",
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        //  "http://localhost:8080/api/v1/suppliers/orange"
          `${process.env.REACT_APP_API_KEY}/api/v1/suppliers/orange`
        );
        setApiData(response.data.content);
        console.log(response.data.content,"datatest");
        setLoading(false);
      } catch (error) {
    ...
    >
      <Flex px="25px" justify="space-between" align="center">
        <Text
          ...
          lineHeight="100%"
        >
          La liste des numéros de Orange
        </Text>
        {/* <Menu /> */}
      </Flex>
